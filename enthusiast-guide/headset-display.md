...
* Überprüfen Sie die Leistung und Stabilität Ihres PCs:
    ...
    * Überprüfen Sie die Protokolle "Application" und "System" in **Ereignisanzeige > Windows-Protokollen** , um festzustellen, ob Sie über eine APP verfügen, die häufig abstürzt und Windows-Fehlerberichterstattung (wer)-Berichte erzeugt.
...
* Überprüfen der APP-und Spiel Stabilität:
    * Stellen Sie sicher, dass Ihr PC die minimalen Systemanforderungen erfüllt, um apps oder Spiele auszuführen, die nicht ordnungsgemäß ausgeführt werden.
...
* Grafikkarten-und Treiber Kompatibilität überprüfen:
    ...
    * Wenn Ihr PC sowohl über eine integrierte Grafikverarbeitungseinheit (igpu) als auch über eine diskrete Grafikverarbeitungseinheit (dGPU) verfügt, stellen Sie sicher, dass Sie mit dem HDMI-Port von dGPU verbunden sind.
...
## <a name="my-headset-displays-turn-blue-and-then-mixed-reality-portal-re-initializes"></a>Mein Headset zeigt Turn Blue an, und das Mixed Reality-Portal wird erneut initialisiert.
...
## <a name="my-headset-displays-are-very-choppy-but-mixed-reality-portals-preview-window-appears-fine"></a>Meine Headset-Anzeige ist sehr viel schimig, aber das Vorschau Fenster des gemischten Reality-Portals erscheint gut.
...
Das System ist möglicherweise nicht in der Lage, Inhalte für das Headset zu Renderingsystem, oder es treten Probleme auf. Überprüfen Sie Folgendes:

1. Öffnen Sie den Task-Manager, um sicherzustellen, dass Ihr PC über ausreichend Compute-Ressourcen verfügt. Sie sollten 80% CPU-Kapazität, 400 MB RAM und Datenträger-e/a unter 80% haben.
