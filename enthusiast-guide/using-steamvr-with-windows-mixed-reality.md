...
description: Erfahren Sie, wie Sie Aufsatz-Headsets und Controller Windows Mixed Reality kompatiblen PCs einrichten und spielen.
ms.topic: article
...
6. Gehen Sie genauso vor, Windows Mixed Reality für SteamVR und alle Mit SteamVR kompatiblen Apps, die Sie auf dem Ziel-PC installieren möchten.
...
![Auf dem Ziel-PC Windows Mixed Reality Dies sind die Windows Mixed Reality Für Dies](images/steamvr-install-files.png)

2. Öffnen Sie eine Eingabeaufforderung, um sicherzustellen, dass sich "SteamVR" und "MixedRealityVRDriver" im gleichen Ordner befinden. Für dieses Beispiel gehen wir davon aus, dass sich der enthaltende Ordner unter *C:\SteamVRInstall befindet.* In diesem Fall sollten Sie an der Eingabeaufforderung Folgenden ausführen:
```powershell
...
4. Zum Ausführen von SteamVR sollten Sie auf die Datei "vrstartup.exe" unter *SteamVR\bin\win64\vrstartup.exe* oderSteamVR\bin\win32\vrstartup.exe *doppelklicken,* wenn auf dem Ziel-PC eine 32-Bit-Version von Windows.
...
**Tipp:** Verwenden Sie die Desktop-App (**Start > Desktop**), um Ihre PC-Desktops innerhalb des Desktops Windows Mixed Reality.

## <a name="using-motion-controllers-with-steamvr"></a>Verwenden von Motion-Controllern mitHilfe von SteamVR
...
So werden Sie beitreten:
  ...
  3. Wählen Sie die **Registerkarte Betas** aus.
  4. Aktivieren Sie **"Beta – öffentliche Betaversion",** und wählen Sie **Schließen aus, um** dies zu bestätigen.  Das Feld beta access code (Betazugriffscode) sollte leer gelassen werden. ![ Wechseln Sie im Eigenschaftendialogfeld für SteamVR zur Betaversion von "SteamVR".](images/steamvr-beta.png)

### <a name="windows-insider-program"></a>Windows-Insider-Programm

Windows Mixed Reality ist Teil der Windows 10.  Viele Fehlerbehebungen und Features, die sich auf Die Benutzer von SteamVR auswirken, sind mit dem Windows ausgestattet.  Wenn Sie die neuesten Vorschauversionen Windows 10 testen möchten, empfehlen wir Ihnen, am [Windows Insider Program teil zu nehmen.](https://insider.windows.com)
...
* **Bewegungsvektor:** Erzwingt, dass die Anwendung immer mit halber Framerate mit Bewegungsvektor-Neuprojektion ausgeführt wird.
* **Keine:** Deaktiviert die Bewegungsreprojektion.

**Erwartete visual Artifacts** 

1. Wenn Sie eine Anwendungsauflösung von mehr als 150 % verwenden, treten möglicherweise Unscharfe auf. Bei verwendung der Bewegungsreprojektion wird empfohlen, einen Wert kleiner als 150 % zu verwenden.
2. Kanten oder Text mit starkem Kontrast, insbesondere auf In-Game-HUDs oder Menüs, können aufgrund von Unausschluss vorübergehend verzerrt oder verzerrt aussehen.
3. SteamVR Home und viele andere Spiele, die nicht zuverlässig 50 bis 60 FPS auf Ihrem PC treffen, haben weiterhin schlechte Erfahrungen mit diesem Modus.
4. Es wurde gemeldet, dass einige Spiele mit einer Geschwindigkeit von 50 % oder mit höherer Latenz (Verzögerung) ausgeführt werden. Melden Sie diese Spiele über die [folgenden Windows-Feedback-Hub](filing-feedback.md) Anweisungen.

Anfänglich haben wir experimentelle Unterstützung für die aktuelle Generation von NVidia-GPUs. Wir iterieren und verbessern unsere Unterstützung für die Bewegungsreprojektion für weitere GPUs, und wir freuen uns auf Ihr Feedback.

**Unterstützte GPUs:** Nvidia GeForce GTX1060, AMD RX470 oder höher, mit installierten Windows Mixed Reality kompatiblen Grafiktreibern.

So aktivieren Sie die Bewegungsreprojektion:

1. Stellen Sie sicher, dass Sie sich für die **Windows Mixed Reality für Die Betaversion von SteamVR** entschieden haben, indem Sie die obigen Anweisungen befolgen.
2. Öffnen Sie das Dashboard "SteamVR".
3. Wählen Sie die Schaltfläche auf der linken Seite mit dem Windows Mixed Reality-Logo aus, um **Windows Mixed Reality für die Einstellungen von SteamVR** zu öffnen.
4. Wählen Sie auf der daraufhin angezeigten Benutzeroberfläche die Registerkarte Grafiken aus.
5. Wählen Sie für "Default SteamVR app motion reprojection mode" die Option "Auto" aus, um die automatische Bewegungsreprojektion zu aktivieren.

![Aktivieren von LSR & LSR-Indikators mit SettingsUX](images/settingsux-enable-lsr.gif)

**Bewegungsreprojektionsindikator**

Der Bewegungsreprojektionsindikator hilft bei der Diagnose von Problemen mit dem experimentellen Feature für die automatische Bewegungsreprojektion. Wenn diese Einstellung auf "true" festgelegt ist, wird oben links im Headset während der automatischen Bewegungsreprojektion ein Indikator angezeigt. Die Farbe und Position dieses Indikators entspricht dem aktuellen Bewegungsreprojektionsmodus. Beispiele finden Sie im folgenden Diagramm.

![mvLSR-Indikator](images/mvLSRIndicator.png)

Grün = Bewegungsreprojektion ist deaktiviert, da die Anwendung mit voller Framerate gerendert werden kann.

Cyan = Motion Reprojection ist eingeschaltet, da die Anwendung CPU-gebunden ist.

Blue = Motion Reprojection ist eingeschaltet, da die Anwendung GPU-gebunden ist.

Rot = Bewegungsreprojektion ist deaktiviert, da die Anwendung mit weniger als halber Framerate ausgeführt wird; Versuchen Sie, die Supersampling zu reduzieren, falls aktiviert.

Grün + Cyan + Blau = Bewegungsreprojektion befindet sich im Halbbildmodus, oder die Anwendung hat eine Bewegungsreprojektion angefordert.

## <a name="sharing-feedback-on-steamvr"></a>Teilen von Feedback zu SteamVR

Ihr Feedback ist sehr wichtig, wenn es darum geht, die Windows Mixed Reality zu verbessern. Senden Sie alle Feedback- und Fehler über die [Windows-Feedback-Hub](filing-feedback.md). Befolgen Sie diese Vorschläge, um Ihr Feedback optimal zu gestalten:

1. Geben Sie in Feedback-Hub an, dass Sie unter "Welche Art von Feedback ist es?" ein neues Problem melden. -Abschnitt oben.
...
4. Beschreiben Sie, welches Game oder welche Anwendung Von Ihnen verwendet wurden, wenn Sie auf das Problem stoßen.
5. Fügen Sie Ihrem Feedback einen SteamVR-Systembericht an. Dies bietet weitere Protokolle, die uns bei der Diagnose Ihres Problems helfen können.
    1. Wählen Sie im Fenster "SteamVR" (die kleinen Fenster mit dem Controllerstatus) den Titel aus, um das Menü zu öffnen.
    2. Wählen Sie "Systembericht erstellen" aus.
    3. Speichern Sie in Datei.
    4. Fügen Sie die generierte Datei direkt an Ihren Feedback-Hub Eintrag an.
6. Wenn Ihr Feedback die Leistung von SteamVR betrifft, erfassen Sie eine Mixed Reality Leistungsablaufverfolgung: 
    1. Wählen Sie die Schaltfläche **Mein Problem neu erstellen** aus.
    2. Wählen Sie in der Dropdown-Dropdown-Datei neben "Include data about" (Daten einschließen zu) **Mixed Reality Performance (Leistung)** aus.
    3. Stellen Sie sicher, dass das Spiel ausgeführt wird, und wählen **Sie Erfassen starten** aus.
    4. Verbringen Sie einige Sekunden damit, das Spiel zu spielen, um die Ablaufverfolgung zu erfassen. Erfassen Sie die Ablaufverfolgung nicht für mehr als 10 bis 15 Sekunden, oder sie ist zu groß für die Übermittlung.
    5. Wählen Sie **Capture beenden** aus.
7. Wählen Sie **Übermitteln** aus, nachdem Sie die restlichen Felder ausgefüllt haben.

Wenn Sie Fragen oder Kommentare haben, können Sie uns auch in unserem [Steam-Forum](http://steamcommunity.com/app/719950/discussions/)erreichen.
