...
1. Wenn Sie einen Fehler beheben, schreiben Sie einen Test, um die Fehlerbehebung zu testen und sicherzustellen, dass zukünftige Codeänderungen sie nicht erneut verhindern.
...
Kopieren Sie die Ausgabe, und fügen Sie sie in Ihre Testdatei ein. Mit dem folgenden Skript werden mehrere Tests nacheinander ausgeführt:
...
* Klicken Sie mit der rechten Maustaste auf > Testing > C#-Testskript.
...
Tests sollten im Allgemeinen basierend auf der Klasse, die sie testen, oder dem Szenario benannt werden, das sie testen.
...
Die Platzierung von szenariobasierten Tests ist weniger definiert. Wenn der Test z. B. das gesamte Eingabesystem austestet, sollten Sie es in einen Ordner "InputSystem" im entsprechenden Bearbeitungs- oder Wiedergabemodus-Testordner platzieren.
...
Weitere Methoden dieser util-Klassen finden Sie in den API-Dokumenten von und , da sie regelmäßig erweitert werden, während dem [`TestUtilities`](xref:Microsoft.MixedReality.Toolkit.Tests.TestUtilities) [`PlayModeTestUtilities`](xref:Microsoft.MixedReality.Toolkit.Tests.PlayModeTestUtilities) MRTK neue Tests hinzugefügt werden.
