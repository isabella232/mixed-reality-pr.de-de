...
---
# <a name="mixed-reality-toolkit-profile-configuration-guide"></a>Mixed Reality Toolkit für die Profilkonfiguration
...
> [!NOTE]
> Das Mixed Reality Toolkit sperrt die Standardkonfigurationsbildschirme, um sicherzustellen, dass Sie immer über einen gemeinsamen Startpunkt für Ihr Projekt verfügen, und es wird empfohlen, mit der Definition eigener Einstellungen zu beginnen, während Sich Ihr Projekt weiterentwickelt. Die MRTK-Konfiguration kann während des Wiedergabemodus nicht bearbeitet werden.
...
> [!IMPORTANT]
> DefaultHoloLens2ConfigurationProfile ist für HoloLens 2. Weitere [Informationen finden](../features/profiles/profiles.md) Sie unter Profile.
...
Wenn Sie ein MixedRealityToolkitConfigurationProfile-Objekt ohne das MixedRealityToolkit in der Szene auswählen, werden Sie gefragt, ob das MRTK die Szene automatisch für Sie einrichten soll. Dies ist optional, es muss jedoch ein aktives MixedRealityToolkit-Objekt in der Szene geben, um auf alle Konfigurationsbildschirme zugreifen zu können.
...
- [Mixed Reality Toolkit für die Profilkonfiguration](#mixed-reality-toolkit-profile-configuration-guide)
  ...
  - [Einstellungen für controllervisualisierung](#controller-visualization-settings)
...
Das Mixed Reality-Projekt bietet ein stabiles und gut trainiertes Eingabesystem zum Weiterleiten aller Eingabeereignisse um das Projekt herum, das standardmäßig ausgewählt ist.
...
Das Mixed Reality-Projekt bietet ein umfassendes Teleportation-System zum Verwalten von Teleportationsereignissen im Projekt, das standardmäßig ausgewählt ist.
...
Das Mixed Reality-Projekt stellt ein neu erstelltes Räumliches Bewusstseinssystem für die Arbeit mit Raumscansystemen im Projekt zur Verfügung, das standardmäßig ausgewählt ist.
...
Einer der fortgeschritteneren Bereiche des Mixed Reality Toolkits ist die Implementierung des Dienstlocatormusters, die die Registrierung eines beliebigen "Diensts" beim Framework ermöglicht. [](https://en.wikipedia.org/wiki/Service_locator_pattern) Dadurch kann das Framework problemlos um neue Features/Systeme erweitert werden, aber auch Projekte können diese Funktionen nutzen, um ihre eigenen Laufzeitkomponenten zu registrieren.
...
| Drei Dof-Drehungen | Quaternion | Nur Drehungseingabe mit 4 Gleitkommaachsen | Ein Drei-Grad-Stilcontroller, z. B. Oculus Go-Controller |
| Sechs Dof | Mixed Reality Pose (Vector3, Quaternion) | Eine Eingabe im Positions- und Drehformat mit Vector3- und Quaternion-Komponenten | Ein Bewegungscontroller oder Zeiger |
...
Eingabeaktionsregeln bieten eine Möglichkeit, ein Ereignis, das für eine Eingabeaktion ausgelöst wird, automatisch in verschiedene Aktionen basierend auf seinem Datenwert zu übersetzen. Diese werden nahtlos innerhalb des Frameworks verwaltet und verursachen keine Leistungskosten.

Konvertieren Sie z. B. das Eingabeereignis mit einer einzelnen dualen Achse von einem DPad in die 4 entsprechenden Aktionen "Dpad Nach oben", "DPad Nach unten" / "Dpad Links" / "Dpad Rechts" (wie in der folgenden Abbildung dargestellt).
...
Zeiger werden verwendet, um die Interaktivität in der Szene von jedem Eingabegerät aus zu bewegen, was sowohl einen Richtungs- als auch einen Treffertest mit jedem Objekt in einer Szene (das einen Collider angefügt hat oder eine Benutzeroberflächenkomponente ist) gibt. Zeiger werden standardmäßig automatisch für Controller, Headsets (Anvieren/Fokus) und Maus-/Toucheingaben konfiguriert.
...
- Zeigende Raycastebenenmasken: Bestimmt, für welche Ebenen Zeiger raycasten.
...
- Prefab für Anvistikcursor: Erleichtert die Angabe eines globalen Anvistikcursors für jede Szene.

Es gibt eine zusätzliche Hilfsschaltfläche, mit der Sie schnell zum Anvinganbieter springen können, um bei Bedarf einige bestimmte Werte für das Anvieren zu überschreiben.
...
> [!NOTE]
> Die aktuelle Gestenimplementierung gilt nur für HoloLens und wird für andere Systeme verbessert, wenn sie dem Toolkit in Zukunft hinzugefügt werden (noch keine Datumsangaben).
...
## <a name="speech-commands"></a>Speech-Befehle

Wie Gesten bieten einige Laufzeitplattformen auch intelligente "Spracherkennung" mit der Möglichkeit, Befehle zu generieren, die von einem Unity-Projekt empfangen werden können. Mit diesem Konfigurationsprofil können Sie Folgendes konfigurieren:

1. Allgemeine Einstellungen: Wenn "Startverhalten" auf "Automatischer Start" oder "Manueller Start" festgelegt ist, wird festgelegt, ob KeywordRecognizer beim Start des Eingabesystems initialisiert werden soll oder ob das Projekt entscheiden kann, wann das KeywordRecognizer initialisiert werden soll. "Recognition Confidence Level" wird verwendet, um die [KeywordRecognizer-API](https://docs.unity3d.com/ScriptReference/Windows.Speech.KeywordRecognizer-ctor.html) von Unity zu initialisieren.
...
> [!IMPORTANT]
> Das System unterstützt derzeit nur Sprache, wenn es auf Windows 10-Plattformen ausgeführt wird, z. B. HoloLens und Windows 10 Desktop, und wird für andere Systeme verbessert, da sie in Zukunft dem MRTK hinzugefügt werden (noch keine Datumsangaben).
...
## <a name="controller-visualization-settings"></a>Einstellungen für controllervisualisierung
...
Die Gemeinsame Nutzung des Tiefenpuffers mit einigen Mixed Reality-Plattformen kann die [Hologrammstabilisierung verbessern.](../performance/hologram-stabilization.md) Beispielsweise kann die Windows Mixed Reality-Plattform die gerenderte Szene pro Pixel ändern, um während der Zeit, die zum Rendern eines Frames gezeitet hat, kleine Kopfbewegungen zu berücksichtigen. Diese Techniken erfordern jedoch Tiefenpuffer mit genauen Daten, um zu wissen, wo und wie weit die Geometrie vom Benutzer entfernt ist.

Um sicherzustellen, dass eine Szene alle erforderlichen Daten im Tiefenpuffer rendert, können Entwickler im Konfigurationsprofil unter *Editoreinstellungen* das Feature *Render depth buffer* (Tiefenpuffer rendern) umschalten. Dadurch wird der aktuelle Tiefenpuffer als Farbe für die Szenenansicht gerendert, indem der Nachbearbeitungseffekt auf die [`DepthBufferRenderer`](xref:Microsoft.MixedReality.Toolkit.Rendering.DepthBufferRenderer) Hauptkamera anwenden wird.

![Hilfsprogramm für Tiefenpuffer rendern Der blaue Zylinder in der Szene verfügt über ein Material mit ](../features/images/mixed-reality-toolkit-configuration-profile-screens/MRTK_DepthBufferExample.gif)
 <sup>deaktiviertem ZWrite, sodass keine Tiefendaten geschrieben werden</sup>
...
1. Schalter für das Pre MRTK-Initialisierungsprofil: Ersetzen Sie beim Start, bevor das MRTK initialisiert und das Profil aktiv wird, das noch nicht in Gebrauch war, um verschiedene Features basierend auf den Gerätefunktionen zu aktivieren/deaktivieren.  Wenn die Umgebung beispielsweise in VR ohne Hardware für die räumliche Zuordnung ausgeführt wird, ist es wahrscheinlich nicht sinnvoll, die Komponente für die räumliche Zuordnung zu aktivieren.
1. **Aktiver Profilwechsel:** Tauschen Sie nach dem Start, nachdem das MRTK initialisiert und ein Profil aktiv wurde, das aktuell verwendete Profil aus, um das Verhalten bestimmter Features zu ändern. Es kann z. B. eine bestimmte untergeordnete Beerfahrung in der Anwendung geben, die möchte, dass die zeigerweiten Zeiger vollständig entfernt werden.

### <a name="pre-mrtk-initialization-profile-switch"></a>Pre MRTK-Initialisierungsprofilschalter
...
Im Prozess zum Wechseln des Profils bleibt die vorhandene UI-Kamera unverändert, um sicherzustellen, dass Komponenten der Unity-Benutzeroberfläche, die Canvas erfordern, nach dem Wechsel weiterhin funktionieren.
